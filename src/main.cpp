...
void loadData(Cube &cube, std::vector<GLuint> &vao)
{
	// Loading cube data
...
void render(std::vector<GLuint> &vao, std::vector<GLuint> &shaders, Cube cube)
{
...
Control tick(sf::Window &window, Cube &cube, Control &controlState)
{
...
void handleEvents(sf::Window &window, sf::Event event, Cube &cube)
{
	// Scale speed
	...
	switch(event.type)
	{
		...
		case sf::Event::MouseWheelMoved:
			...
			cube.scale(wheelMovement * scaleSpeed);
			break;
	}	
}
...
int main()
{
	...
	cube.move(glm::vec3 (-1.0, -1.0, -3.0));
